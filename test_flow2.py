...
with sync_playwright() as p:
    ...
    print("Logged in successfully. At:", page.url)
    
    # Navigate to operations or cases if needed, or find the first case link
    page.goto("http://localhost:5173/dashboard")
    page.wait_for_load_state("networkidle")
    
    # Click the first case link
    page.click("a[href*='/cases/']:first-child")
    page.wait_for_load_state("networkidle")
    print("Entered case workspace. At:", page.url)
    
    # Click Secure Document Ingestion
    page.click("button:has-text('SECURE'), button:has-text('UPLOAD'), .upload-button")
